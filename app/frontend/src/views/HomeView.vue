...
onMounted(() => {
  if (store.books.length === 0) {
    store.searchBooks('popular fiction')
  }
})
</script>
...
<template>
  <main class="flex flex-col gap-6 p-6">
    ...
    <!-- Loading State -->
    <div v-if="store.isLoading" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
      <div
        v-for="n in 20"
        :key="n"
    ...
    <div v-else-if="store.error" class="flex flex-col items-center gap-3 py-20 text-center">
      ...
      <Button label="Try Again" severity="secondary" @click="store.searchBooks(store.searchQuery)" />
    </div>

    <!-- Empty State -->
    <div v-else-if="!store.books.length && store.searchQuery" class="flex flex-col items-center gap-3 py-20 text-center">
    ...
    <!-- Book Grid -->
    <div v-else class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 items-stretch">
      <BookCard
    ...
    <!-- Pagination -->
    <div v-if="!store.isLoading && store.books.length" class="flex items-center justify-center gap-4 pt-2">
      <Button
