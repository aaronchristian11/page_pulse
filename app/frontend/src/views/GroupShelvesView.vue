...
const activeBookKeys = computed(() =>
    new Set(groups.activeGroupBooks.map((e) => e.key))
)
...
<template>
    ...
    <main class="flex flex-col gap-6 p-6 max-w-full">
        ...
        <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            ...
            <Button v-else label="Sign in to create a group" icon="pi pi-user" severity="secondary" outlined @click="router.push('/login')" />
        </div>
                    ...
                    <template #content>
                        <div class="flex flex-col gap-1">
                            ...
                            >
                                ...
                                <Tag
                                    v-if="group.currentUserRole === 'Administrator'"
                                    value="Admin"
                                    severity="warn"
                                    class="text-xs"
                                />
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            >
                                <Avatar
                                    ...
                                    shape="circle"
                                    :class="member.role === 'Administrator' ? 'bg-amber-100 text-amber-700 dark:bg-amber-900 dark:text-amber-200' : 'bg-surface-100 dark:bg-surface-800 text-color'"
                                />
                                ...
                                <div class="flex flex-col gap-4 sm:flex-row sm:items-start">
                                    <div class="flex gap-4 flex-1 min-w-0">
                                        <div class="min-w-0 flex-1">
                                            <p class="font-semibold text-color font-mono text-sm">{{ entry.key }}</p>
                                            <Tag
                                    ...
                                    <div class="flex gap-2 sm:ml-auto sm:flex-shrink-0">
                                        <Button
                            ...
                            <template #content>
                                <div class="flex h-full flex-col gap-3">
                                    ...
                                    <div class="mt-auto flex flex-col gap-2">
                                        ...
                                        />
                                        <Button
                                            v-if="!activeBookKeys.has(book.id)"
                                            label="Add to shelf"
                                            icon="pi pi-plus"
                                            size="small"
            ...
            <Card v-if="!groups.groups.length" class="border border-dashed border-surface-300 dark:border-surface-700">
                <template #content>
                    <div class="flex flex-col items-center gap-3 py-8 text-center">
                        ...
                        <Button v-if="auth.user" label="Create a Group" icon="pi pi-plus" @click="createDialogVisible = true" />
                    </div>
                </template>
            </Card>
        </div>
    ...
    <Dialog v-model:visible="createDialogVisible" modal header="Create a new group" class="w-[92vw] max-w-lg">
        <div class="flex flex-col gap-4 pt-2">
            ...
            <div class="flex flex-col gap-2">
                ...
                <Textarea id="group-desc" v-model="createGroupDesc" rows="3" autoResize placeholder="What kind of books does this group focus on?" class="w-full" />
            </div>
    ...
    <!-- ── Add Book Dialog ──────────────────────────────────────────────────── -->
    <Dialog v-model:visible="bookDialogVisible" modal :header="bookDialogBook ? `Add &quot;${bookDialogBook.title}&quot;` : 'Add a book'" class="w-[92vw] max-w-xl" @hide="closeBookDialog">
        <div class="flex flex-col gap-4 pt-2">
    ...
    <Dialog v-model:visible="addMemberDialogVisible" modal header="Add a member" class="w-[92vw] max-w-md">
        <div class="flex flex-col gap-4 pt-2">
            <div class="flex flex-col gap-2">
                ...
                <InputText id="add-member-username" v-model="addMemberUsername" placeholder="Enter their username" class="w-full" @keydown.enter="submitAddMember" />
            </div>
            <div class="flex justify-end gap-2">
