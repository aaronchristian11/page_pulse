...
export const openLibraryApi = {
  ...
  search(query: string, options: SearchOptions = {}): Promise<SearchResponse> {
    ...
    return apiFetch<SearchResponse>('/search.json', {
      [field]: query,
      page,
      limit,
  ...
  getWork(workId: string): Promise<WorkDetail> {
    return apiFetch<WorkDetail>(`/works/${workId}.json`)
  },
  ...
  getAuthor(authorId: string): Promise<AuthorDetail> {
    return apiFetch<AuthorDetail>(`/authors/${authorId}.json`)
  },
  ...
  getSubject(subject: string, limit = 12): Promise<{ works: BookSearchResult[] }> {
    return apiFetch(`/subjects/${encodeURIComponent(subject)}.json`, { limit })
  },
