...
export const register = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const role = await knex('roles').whereILike('role', '%general%').first();
        const permission = await knex('permissions').whereILike('permission', '%manage shelf%').first();
        await knex('user_role_permissions').insert({ id, role_id: role.id, permission_id: permission.is })
